...
def banner():
    ...
    print(f"""
                    ...
                    ({Fore.LIGHTWHITE_EX}2{Fore.WHITE}) Phone Number Bomber
    ...
    if choice == "1":
        i = 0
        try:
            counter = input(f"  ({Fore.LIGHTWHITE_EX}>{Fore.WHITE}) Number of mails → ")
...
if __name__ == "__main__":
    with open('users.json') as f:
        data = json.load(f)
        name = data["username"]
    ctypes.windll.kernel32.SetConsoleTitleW(f"Wave Bomber | Logged in as {name}")
    # os.system(f"title WaveBomber | Logged in as {name}")
    banner()